    ...
    def get_action(self, env):
        ...
        if self.mistake_timer > 0:
            ...
            if self.mistake_type == 'under': 
                final_steer = expert_steer * 0.3
            elif self.mistake_type == 'over': 
                final_steer = np.clip(expert_steer * 1.7, -1, 1)
            elif self.mistake_type == 'late': 
                final_steer = expert_steer * 0.2 if self.mistake_timer < 8 else np.clip(expert_steer * 1.5, -1, 1)
            else:
