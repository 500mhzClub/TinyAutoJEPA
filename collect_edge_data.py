...
class EdgeExplorationDriver:
    def __init__(self):
        self.prev_idx = 0; self.has_reset = False; self.edge_bias = 0.0; self.bias_timer = 0
    ...
    def get_action(self, env, obs_raw):
        ...
        if self.bias_timer > 100:
            ...
            if black_prox < 100: 
                final_steer = np.clip(expert_steer + self.edge_bias, -1.0, 1.0); gas = 0.5
            else:
                final_steer = expert_steer; gas = 0.3
            if self.bias_timer > 125: self.bias_timer = 0
        else:
            final_steer = expert_steer; gas = 0.5 if speed < 25 else 0.3
        return np.array([final_steer, gas, 0.0], dtype=np.float32)

    def reset(self):
        self.prev_idx = 0; self.has_reset = False; self.bias_timer = 0
...
def worker_func(worker_id):
    ...
    for episode in range(EPISODES_PER_WORKER):
        ...
        for step in range(MAX_STEPS):
            ...
            obs_raw = next_obs_raw; obs = next_obs
...
if __name__ == "__main__":
    ...
    print(f"Launching {NUM_WORKERS} Edge Explorers...")
