...
class PurePursuitDriver:
    ...
    def get_action(self, env):
        ...
        # 4. Predictive Speed Control (Lookahead Curvature)
        future_curve = self._estimate_curvature(track_coords, closest_idx, int(speed * 0.8))
        
        base_target = 40.0
        # Slow down significantly for upcoming curves
        target_speed = max(15.0, base_target - (future_curve * 30.0))
        speed_error = target_speed - speed
        
        if speed_error > 2:
            gas, brake = 0.6, 0.0
        elif speed_error < -5:
            gas, brake = 0.0, min(0.5, abs(speed_error)/15.0)
        else:
            gas, brake = 0.3, 0.0
        ...
        return np.array([steer, gas, brake], dtype=np.float32), speed, track_coords[closest_idx], target_pt
...
def run_visual_verification():
    """Run a single episode with rendering to verify driver behavior."""
    print("--- VISUAL VERIFICATION MODE ---")
    print("Watching Expert Driver with Predictive Speed Control...")
    ...
    while True:
        try:
            action, speed, _, _ = driver.get_action(env)
        except:
        ...
        if terminated or truncated:
            print("Lap finished or crashed. Resetting...")
            obs, _ = env.reset()
            driver.reset()
            for _ in range(50): env.step(np.array([0,0,0]))
...
def worker_func(worker_id):
    ...
    for episode in range(EPISODES_PER_WORKER):
        ...
        for step in range(MAX_STEPS):
            ...
            # Strict safety cut-off for data quality
